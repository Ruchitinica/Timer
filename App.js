...
import { StyleSheet, Text, View, disableYellowBox, TouchableOpacity} from 'react-native';
  ...
  return (
    <View style={styles.container}>
      ...
      <LinearGradient
        ...
        colors={['rgb(100, 210, 227)', 'rgb(21, 24, 194)']}
        style={{
          position:'absolute',
          left: 0,
          right:0,
      ...
      <View style={{flexDirection:'row'}}>
        {
          alarmeSound.map(function (val){
            if(val.selecionado){
            return (
            <TouchableOpacity onPress={()=>setarAlarme(val.id)} style={styles.btnEscolherSelecionado}><Text style={{ color: 'white' }}>{val.som}</Text></TouchableOpacity> 
            );

            }else{
              return(
                <TouchableOpacity onPress={()=>setarAlarme(val.id)} style={styles.btnEscolher}><Text style={{ color: 'white' }}>{val.som}</Text></TouchableOpacity>);
        ...
        //TODO : na proxima aula trabalhamos a lógica do timer.  
        return(
...
const styles = StyleSheet.create({
  ...
  container: {
    flex: 1,
    backgroundColor: 'rgb(235, 52, 229)',
    alignItems: 'center',
  ...
  btnEscolher:{
    marginRight:10,
    padding:8,
    backgroundColor:'rgb(20, 135, 126)'
  },
  btnEscolherSelecionado: {
    marginRight: 10,
    padding: 8,
    backgroundColor: 'rgba(20, 135, 126,0.3)',
    borderColor:'white',
    borderWidth:1,
  }
});
